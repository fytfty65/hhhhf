            ...
            <div className="rounded border border-sky-400/30 bg-sky-500/5 p-2">
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {globalSelection.map((name) => {
                  ...
                  return <div key={name} className="rounded bg-white/60 p-1.5 dark:bg-slate-950/30"><div className="truncate text-[10px] font-bold text-slate-700 dark:text-slate-200">{name}</div><div className="mt-1 flex items-baseline justify-between"><span className="text-sm font-black text-slate-900 dark:text-white">{summary.cii}</span><span className="text-[9px] text-slate-500">{summary.riskLevel}</span></div><div className="mt-1 text-[9px] leading-4 text-slate-500">信号 {summary.availableSignals}/{summary.totalSignals} · {summary.estimated ? '估算' : '实测'}<br />{summary.source} · {summary.freshness}</div><div className="mt-1.5 space-y-0.5 border-t border-slate-300/40 pt-1 dark:border-white/10">{summary.signals.map((signal) => <div key={signal.key} className="flex items-center justify-between gap-1"><span>{signal.label}</span><span className={signal.available ? 'text-emerald-600 dark:text-emerald-300' : 'text-slate-400'}>{signal.available ? `${signal.provider}${signal.estimated ? ' · 估算' : ''}` : '无数据'} · {signal.freshness}</span></div>)}</div></div>;
                })}
              </div>
            </div>
          )}
