...
.DESCRIPTION
  ...
  基线（2026-09-16 拆分前）：
    ...
    首屏 JS               1,115.0 KB（13 chunk）（2026-09-18 更新：核对面板新增「数据降级」
...
# ---- 体积与行数对比（与拆分基线同一算法） ----
Write-Log ""
Write-Log "=== 体积 / 行数对比（基线：3,991 行 / 1,115.0 KB JS / 141.1 KB CSS） ==="
...
if (Test-Path $indexHtml) {
  ...
  Write-Log "  （首屏 JS 不得超过基线 1,115.0 KB；变大即为劣化，需回滚该批）"
} else {
