...
| 首屏 JS | **1,108.0 KB（13 个 chunk）**（2026-09-17 经用户确认由 1,104.6 → 1,105.0 KB；2026-09-19 更新到 1,108.0 KB（保底路线标记 + 核对数据合并）：核对面板陆续加了「本次调整 / 价格核对 / 跨城怎么走 / 长途分段 / 自动复核 / 诉求核对 / 数据降级」与流式 JSON 解析修复，面板本体走 dynamic 按需加载，差额来自宿主接线） |
...
**验收**：拆分后首屏 JS ≤ **1,108.0 KB**，9 个 E2E 全绿，`ContextualLobby.tsx` 只剩状态与编排。
