...
## 💰 B 项实测数据（LLM 成本，2026-09-19，`work/probe_prompt_cost.py`）

用探针包住 LLM 客户端跑**真实推演**，统计调用次数与 prompt 体积：

| 场景 | 调用次数 | 总字符 | 关键发现 |
|---|---|---|---|
| 洛阳 2 天 | **1** | 5,825 | 单次请求内长消息重复 = **0**（"巨型 prompt 在同一请求里重复发送"**未复现**） |
| 新疆 30 天 | **6** | **55,291** | 每次调用都重发**同一条 8,932 字符** system prompt；**≈97% 的输入字符是这段 prompt 的重复** |

结论：真正的成本乘数不是"同请求内重复"，而是**长途分段生成（≥14 天、最多 6 段）把整套
system prompt 发了 6 次**。修法（按性价比排序，均需先测量后改）：
1. **分段 prompt 瘦身**：每段只带本段需要的规则与候选子集（目标 8,932 → ~3,000 字符，预计省 60%+ 输入）；
2. **前缀缓存**：若 DashScope 兼容口支持上下文缓存，把稳定前缀交给它缓存（需实测该账号是否生效）；
3. **失败重试降级**：`attempt 1/3` 重试时改用更小的 prompt，而不是整段重发；
4. `max_tokens=16384` 已收敛到 `core/llm_config`，可按天数分档（当前全场景同一个上限）。

A 项已完成：`core/llm_config.py` 是模型名/端点/temperature/max_tokens 的**唯一事实来源**
（消灭 `agent.py` `qwen-turbo` vs `conflict.py` `qwen2.5:0.5b` 两套兜底 + `conflict.py` 兜底本地
Ollama 端点）；并有**静态契约单测**防止再次出现"各自写一套兜底"。
...
**补充证据（第二次探针，2026-09-19 晚）**
- 注册 token ≠ 登录 token（各自新建会话，正常）；用**登录** token 走的全流程，上传同样 200、
  `uploader_id` 同样等于当前用户 id —— 所以问题**不在**"注册写坏了会话"。
- 🔴 **两次运行上传的 photo id 完全相同**（`d263dedc…`）：内容寻址去重生效；
  于是**第二个上传同样图片的人**，响应里被告知 `uploader_id` 是他自己，但索引里那条记录仍属**第一个上传者**
  （`Index.Add` 遇到同 id 直接 return nil，不更新归属）→ 他既看不到也取不到"自己"的照片。
  **这至少是一个确定的产品 bug**：响应在骗人 + 第二个上传者被锁在外面。
  修法（待做）：记录里改成 `uploader_ids []string`（或 `shared_with`），`VisibleTo`/`ListMine` 查这个列表；
  上传命中已有内容时**追加**上传者而不是丢弃。
- 第一次运行（记录是全新的、同一个 token、上传后立刻查）也是 0 张 / 本人 403 —— 这一条**去重解释不了**，
  说明**很可能还有第二个身份解析问题**。两者都需要**运行时证据**才能定性：
  在 `/my/photos` 响应里加 `debug_user_id` + 记录 `uploader_id`（需用户重启一次），
  或我在自己进程里另起一个 gateway 实例自由插桩。
...
| 首屏 JS | **1,116.0 KB（13 个 chunk）**（2026-09-17 经用户确认由 1,104.6 → 1,105.0 KB；2026-09-19 更新到 1,116.0 KB（保底路线标记 + 核对数据合并）：核对面板陆续加了「本次调整 / 价格核对 / 跨城怎么走 / 长途分段 / 自动复核 / 诉求核对 / 数据降级」与流式 JSON 解析修复，面板本体走 dynamic 按需加载，差额来自宿主接线） |
...
**验收**：拆分后首屏 JS ≤ **1,116.0 KB**，9 个 E2E 全绿，`ContextualLobby.tsx` 只剩状态与编排。
...
| F22 | 地图**只拿当天节点**导致"当天为空即整块空白"：改为当天为空时回退整个行程（并写明"这里是整个行程的 N 个节点"）+ 空状态自诊断（本天/全部/阶段） | `bcf58cb` |
| F23 | drafting/deduction 阶段不再显示"底图不可达"提示（那时还没有路线可看） | `e803c4b` |
| F24 | **CSP 把地图瓦片全拦掉**（瓦片域名只在 `img-src`、不在 `connect-src`，而 MapLibre 用 fetch 取瓦片）→ 修复 + `app/lib/csp.test.ts` 契约测试 + E2E「瓦片请求必须发出」守卫；顺带修两处降级判断漏洞 | `a9538e7` |
